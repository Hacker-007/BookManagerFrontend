  ...
  <div>
    ...
    <div v-if="ifRolesPresent()">
      ...
      <div v-else>
        ...
        <h1>Title:</h1>
        <input type="text" v-model="checkinTitle">
        <button @click="checkIn($parent.$store.state.user.username, checkinTitle)">Checkin A New Book</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkIn(username, title) {
      let _self = this;
      fetch(
        "http://bookmanager.us-east-2.elasticbeanstalk.com/bookmanager/student/checkin/" +
          username +
          "/" +
          title,
        {
